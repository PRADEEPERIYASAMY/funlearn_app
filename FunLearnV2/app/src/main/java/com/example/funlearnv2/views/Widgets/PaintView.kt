...
class PaintView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private val captureImage: Bitmap? = null
    private var originalImage: Bitmap? = null
    private val rotateImage: Bitmap? = null
...
/*Drawable drawable = getResources ().getDrawable ( R.drawable.ic_rotate_right_black_24dp );
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                refY = y
                if (toMove) {
                    toResize = if (isToResize(refX, refY)) {
                        true
                    } else {
                        false
                    }
            ...
            MotionEvent.ACTION_MOVE -> if (!toMove) {
                ...
                if (toResize) {
                    ...
                    (leftImage += nX - refX.toInt()).toInt()
                    (topImage += nY - refY.toInt()).toInt()
                }
                refX = nX
                refY = nY
                invalidate()
    ...
    private fun isToResize(refX: Float, refY: Float): Boolean {
        return if (refX >= leftImage && refX < leftImage + image!!.width && (refY >= topImage && refY <= topImage + 20 || refY >= topImage + image!!.height - 20 && refY <= topImage + image!!.height)) {
            true
        } else false
    }
